 ...
 */
    ...
    let studentKeys = Object.values(student);
...
/**
 * Crea un código para llenar un objeto vacio:
 * 
 * const objeto = {}
 * 
...
const existePropiedad = (nombrePro, obj = new Object())=> {
    return  obj.hasOwnProperty(nombrePro);
}
...
console.log(existePropiedad("email", obj));
...
'use strict';
    ...
    let myObject = {
        nombre: "Marco",
        agregarProp: function(){
            this.edad = 57;
        }
    }

    // myObject.agregarProp("edad", 57)
...
let cadena = "";
for(let i = 0; i < simpleExercise.length; i++){

    cadena += `${simpleExercise[i]["a"] + simpleExercise[i]["b"]} ${simpleExercise[i]["name"]}`
    if ( i + 1 === simpleExercise.length) break;
    cadena += "-=- " 
